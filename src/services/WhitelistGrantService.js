...
class WhitelistGrantService {
  ...
  /**
   * Extend whitelist duration for a user
   * @param {Object} params
   * @param {string} params.steamid64 - Steam ID64
   * @param {number} params.months - Number of months to extend
   * @param {string} params.granted_by - Discord ID of granter
   * @returns {Promise<Object>} Extension result
   */
  async extendWhitelist({ steamid64, months, granted_by }) {
    try {
      // Validate Steam ID
      ...
      if (!existingEntry) {
        return {
          success: false,
          errors: ['User not found in whitelist system']
        };
      }

      // Create extension entry (stacks with existing)
      const extensionEntry = await this._createWhitelistEntry({
        steamid64,
        ...
        metadata: {
          ...
          extendedFrom: existingEntry.id
        }
      });

      loggerConsole.info('Whitelist extended', {
        steamid64,
        months,
        granted_by,
        newExpiration: extensionEntry.expiration
      });
      ...
      loggerConsole.error('Whitelist extension failed', {
        steamid64,
        months,
        error: error.message
      });
