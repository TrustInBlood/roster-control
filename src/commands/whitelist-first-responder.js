...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('whitelist-first-responder')
    .setDescription('Grant 6-month first responder whitelist and role')
  ...
  async execute(interaction) {
    await permissionMiddleware(interaction, async () => {
      try {
        ...
        loggerConsole.info('First responder whitelist grant initiated', {
        ...
        const successEmbed = createResponseEmbed({
          title: '🚑 First Responder Whitelist Granted',
          description: 'First responder whitelist access has been granted successfully!',
          fields: [
            { name: 'Discord User', value: `<@${discordUser.id}>`, inline: true },
            { name: 'Steam ID', value: steamid, inline: true },
            { name: 'Type', value: 'First Responder', inline: true },
        ...
        try {
          const publicEmbed = createResponseEmbed({
            title: '🚑 First Responder Whitelist Granted',
            description: `<@${discordUser.id}> has been granted **First Responder** whitelist access${result.roleAssigned ? ' and Discord role' : ''}`,
            fields: [
              { name: 'Duration', value: '6 months', inline: true },
              { name: 'Granted By', value: `<@${interaction.user.id}>`, inline: true }
            ],
        ...
        loggerConsole.error('First responder whitelist grant failed', {
          error: error.message,
          stack: error.stack
        });
        await sendError(interaction, `Failed to grant first responder whitelist: ${error.message}`);
      }
    });
  }
};
